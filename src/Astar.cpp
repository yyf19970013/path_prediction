...
Path Astar::run(const Map& map, const coordinate& start, const coordinate& goal)
{
    ...
    while(!open_list.empty())
    {
        ...
        for(const auto& neighbor : getNeighbors(map, curr))//这里计算的就是能走的邻居
        {
            /*
            存在到处走不通的
            */
...
{   
    ...
    for(int i = 0; i < 4; ++i)
    {
        ...
        else if(i == 1 && curr_coordinate.y + 1 <= common_.getCol() - 1  && get<1>(maproute) == true)
        {
            neighbors.emplace_back(coordinate(curr_coordinate.x, curr_coordinate.y + 1));
        }
        else if(i == 2 && curr_coordinate.x + 1 <= common_.getRow() - 1 && get<2>(maproute) == true)
        {
            neighbors.emplace_back(coordinate(curr_coordinate.x + 1, curr_coordinate.y));
        }
...
sidePath Astar::updateSidePath(const Path& path, const vector<size_t>& conner_index)
{
    sidePath spath;
